...
console.log("// 5. Escreva um programa que calcula o Índice de Massa Corporal (IMC) de uma pessoa e" +
    ...
    let alturaEX5 = Number(prompt('[Digite um número para altura em metros '));
    let IMC = (pesoEX5 / (alturaEX5**2)).toFixed(3);
    if (IMC < 18.5) {
...
console.log("// 7. As maçãs custam R$ 0,30 se forem compradas menos do que uma dúzia, e R$ 0,25 se" +
    ...
    let qtdEX7 = Number(prompt('[Digite uma quantidade]'))
    let precoUnitarioEX7 = undefined;
    
    qtdEX7 > 12 ? precoUnitarioEX7 = 0.25 : precoUnitarioEX7 = 0.3;
    console.log(`O valor da compra foi R$${(precoUnitarioEX7 * qtdEX7).toFixed(2)} com valor unitário por maçã de R$ ${precoUnitarioEX7.toFixed(2)}`);
